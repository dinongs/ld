...
/**
 * 16 * @author Joshua 17 * 用法： 18 * DBHelper dbHelper = new DBHelper(this); 19
 * * dbHelper.createDataBase(); 20 * SQLiteDatabase db =
 * dbHelper.getWritableDatabase(); 21 * Cursor cursor = db.query() 22 *
 * db.execSQL(sqlString); 23 * 注意：execSQL不支持带;的多条SQL语句，只能一条一条的执行，晕了很久才明白 24 *
 * 见execSQL的源码注释 (Multiple statements separated by ;s are not supported.) 25 *
...
public class DBHelper extends SQLiteOpenHelper {
	...
	/*
	 * 34 //如果你想把数据库文件存放在SD卡的话 35 private static String DB_PATH =
	...
	/**
	 * 45 * 如果数据库文件较大，使用FileSplit分割为小于1M的小文件 46 * 此例中分割为 hello.db.101
	...
	/**
	 * 54 * 在SQLiteOpenHelper的子类当中，必须有该构造函数 55 * @param context 上下文对象 56 * @param
	 * name 数据库名称 57 * @param factory 一般都是null 58 * @param version
	...
	/**
	 * 168 * 该函数是在第一次创建的时候执行， 169 * 实际上是第一次得到SQLiteDatabase对象的时候才会调用这个方法 170
	 */
	@Override
